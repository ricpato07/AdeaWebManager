(function() {
	...
	/**
	 * 
	 * @description Archivo que se encarga de los filtros utilizados dentro del
	 *              sistema
	 */
	angular.module('adeaModule')
			...
			.filter('tipoPerfil', tipoPerfil)
			.filter('tipoUsuario', tipoUsuario)
			.filter('tipoPoliza', tipoPoliza)
			.filter('tipoAplicacion', tipoAplicacion)
			.filter('idUsuario', idUsuario)
			.filter('ordenarListMilisTickets', ordenarListMilisTickets);

	/**
	 * @type {string[]}
	 * @description Filtro que se encarga de colocar un formato '--/--/----' si
	 *              es que la fecha es nula o undefined, si llega con un valor
	 *              se le agrega el filtro de fecha mas el formato solicitado
	 */
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de los estatus dentro del sistema
	 */
	estatus.$inject = [ '$log' ];
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar la fecha en formato de dia, mes,
	 *              año, horas y segundos si llega un valor nulo pone lineas
	 *              para distingien no q hay fecha '--/--/----'
	 */
	 ...
	 * @type {string[]}
	 * @description Filtro encargado de colocar la fecha en formato de dia, mes,
	 *              año si llega un valor nulo pone lineas para distingien no q
	 *              hay fecha '--/--/----'
	 */
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar Empleado de fabrica simpre y
	 *              cuando esl valor llege nulo
	 */
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar los titulos a las formas de pago
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar el formato de los numeros para
	 *              que sean de tipo monead ademas de agregarle el simbolo $ al
	 *              inicio con 2 decimales, y si llega vacio colocar un no
	 *              aplica par aalgunas partes de los segmentos
	 */
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar el formato los porentajes dentro
	 *              del sistema con su simbolo % , si el valor es nulo coloca un
	 *              no aplica
	 */
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar el un valor icono, dependiendo
	 *              del valor que llege si es true una paloma si es falso un
	 *              tache
	 */
	palomaotache.$inject = [];
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar solo la palabra SI si el valor
	 *              que llega es S, NO si es valor es N
	 */
	siono.$inject = [];
	...
	function siono() {
		return function(sino) {
			if (sino === 'S') {
				return 'SI';
			} else if (sino === 'N' || sino == undefined || sino == '') {
				return 'NO';
			}
		}
	}
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar La frase no aplica si es que el
	 *              valor que llega, es nulo undefined
	 */
	noaplica.$inject = [];
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar la palabra dependiedo de que
	 *              tipo de dato es al que se refiere el valor que trae C si es
	 *              caracter, A anfanumerico y N si es Numérico
	 */
	tipoDato.$inject = [];
	...
	/**
	 * @type {string[]}
	 * @description Filtro encargado de colocar el valor numerico en formato de
	 *              positivo ademas de agregar el formato numérico con el
	 *              simbolo $
	 */
	 ...
	 * @description Filtro encargado de los estatus dentro del sistema
	 */
	tipoMovimiento.$inject = [ '$log' ];

	function tipoMovimiento($log) {
		return function(codEstatus) {
			if (codEstatus === 'A') {
				return 'Alta';
			} else if (codEstatus === 'B') {
				return 'Baja';
			} else if (codEstatus === 'M') {
				return 'Modificación';
			} else {
				codEstatus;
			}
	 ...
	 * @description Filtro encargado de los estatus dentro del sistema
	 */
	tipoPerfil.$inject = [ '$log' ];

	function tipoPerfil($log) {
		return function(codEstatus) {
			if (codEstatus === 'A') {
				return 'Administrador';
			} else if (codEstatus === 'C') {
				return 'Solicitud de consulta';
			} else if (codEstatus === 'V') {
				return 'Visualización';
			} else if (codEstatus === 'O') {
				return 'Otros';
			} else {
				codEstatus;
			}
	...
	tipoUsuario.$inject = [ '$log' ];

	function tipoUsuario($log) {
		return function(codEstatus) {
			if (codEstatus === 'A') {
				return 'Aplicativo';
			} else if (codEstatus === 'B') {
				return 'Base de Datos';
			} else if (codEstatus === 'R') {
				return 'Red';
			} else if (codEstatus === 'C') {
				return 'Usuario de correo Electrónico';
			} else if (codEstatus === 'O') {
				return 'Otros';
			} else {
				codEstatus;
	...
	tipoAplicacion.$inject = [ '$log' ];

	function tipoAplicacion($log) {
		return function(codEstatus) {
			if (codEstatus === 'DMS') {
				return 'AdeA DMS';
			} else if (codEstatus === 'MX') {
				return 'AdeA MX';
			} else if (codEstatus === 'DNET') {
				return 'AdeA DMS NET';
			} else if (codEstatus === 'RF') {
				return 'RF';
			} else if (codEstatus === 'R') {
				return 'Restarurado';
			} else {
	...
	idUsuario.$inject = [ '$log' ];

	function idUsuario($log) {
		return function(codEstatus) {
