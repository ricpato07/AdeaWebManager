...
(function(DlhSoft) {
	...
	(function(Controls) {
		...
		(function(KanbanBoard) {
			...
			angular
					.module('DlhSoft.Kanban.Angular.Components', [])
					.directive(
							'dsKanbanBoard',
							function() {
								return {
									...
									controller : function($scope) {
										...
										for (var i = 0; i < this.items.length; i++) {
											...
											if (!item.group
													|| this.groups
															.indexOf(item.group) < 0)
												item.group = this.groups[0];
											if (!item.state
													|| this.states
															.indexOf(item.state) < 0)
										...
										for (var i = 0; i < this.groups.length; i++) {
											...
											if (!group.state
													|| this.groupStates
															.indexOf(group.state) < 0)
												group.state = this.states[0];
											if (group.isCollapsed === undefined)
												group.isCollapsed = group.state ? group.state.isCollapsedByDefaultForGroups
														: false;
										}
										if (!this.itemType)
										...
										if (!this.groupWidth)
											this.groupWidth = '15%';
										if (!this.stateWidth)
											this.stateWidth = ((!this.hideGroups ? 85
													: 100) / this.states.length)
													+ '%';
										if (!this.itemHeight)
											this.itemHeight = 56;
										if (!this.groupHeight)
										...
										if (!this.itemTemplateUrl)
											this.itemTemplateUrl = 'app/lib/kanban/DlhSoft.Kanban.Angular.Components.Templates/kanban-item.html';
										if (!this.groupTemplateUrl)
											this.groupTemplateUrl = 'app/lib/kanban/DlhSoft.Kanban.Angular.Components.Templates/kanban-group.html';
										if (!this.stateTemplateUrl)
											this.stateTemplateUrl = 'app/lib/kanban/DlhSoft.Kanban.Angular.Components.Templates/kanban-state.html';
										...
										this.canDropItem = function(type,
												index, group, state,
												targetIndex) {
											if (type !== KanbanBoard.types.item
													|| targetIndex === index)
												return false;
											var item = _this.items[index];
											if (_this.canMoveItem)
												return _this.canMoveItem({
													item : item,
													index : targetIndex,
													previousIndex : index,
													group : group,
										...
										this.onItemDrop = function(type, index,
												group, state, targetIndex) {
											if (type !== KanbanBoard.types.item
													|| targetIndex === index)
												return;
										...
										this.canDropGroup = function(type,
												index, targetIndex) {
											if (type !== KanbanBoard.types.group
													|| targetIndex === index)
												return false;
											var group = _this.groups[index];
											if (_this.canMoveGroup)
												return _this.canMoveGroup({
													group : group,
													index : targetIndex,
													previousIndex : index
												});
										...
										this.onGroupDrop = function(type,
												index, targetIndex) {
											if (type !== KanbanBoard.types.group
													|| targetIndex === index)
												return;
											...
											_this.groups.splice(targetIndex, 0,
													group);
											if (_this.onGroupIndexChanged)
												_this.onGroupIndexChanged({
													group : group,
													index : targetIndex,
													previousIndex : index
												});
										...
										this.addNewItem = function(group, state) {
											var item = {
												name : _this.newItemName,
												group : group,
												state : state,
												assignedResource : _this.newItemResource
											};
											_this.items.push(item);
											if (_this.onAddingNewItem != null)
												_this.onAddingNewItem({
													item : item
												});
										};
									},
									controllerAs : 'dskb',
									templateUrl : 'app/lib/kanban/DlhSoft.Kanban.Angular.Components.Templates/kanban-board.html'
								};
							})
					.directive(
							'dsKanbanDraggableItem',
							function($timeout) {
								return {
									restrict : 'A',
									scope : {
									...
									link : function(scope, element) {
										var parentElement = scope.isDraggingParent ? element
												.parent()[0]
												: null;
										element = element[0];
										if (!parentElement)
										...
										function onDragStart(event) {
											...
											event.dataTransfer
													.setData(
															'text',
															(scope.dragType ? scope.dragType
																	+ ':'
																	: '')
																	+ scope.dragIndex);
											parentElement.originalOpacity = parentElement.style.opacity;
											$timeout(function() {
												parentElement.style.opacity = (parentElement.originalOpacity ? parentElement.originalOpacity
														: 1) / 2;
											});
										}
										function onDragEnd() {
								...
								return {
									...
									link : function(scope, element) {
										...
										function onDrop(event) {
											var itemInfo = event.dataTransfer
													.getData('text');
											var infoSeparatorIndex = itemInfo
													.indexOf(':');
											var type = infoSeparatorIndex >= 0 ? itemInfo
													.substr(0,
															infoSeparatorIndex)
													: KanbanBoard.types.item;
											var index = parseInt(infoSeparatorIndex >= 0 ? itemInfo
													.substr(infoSeparatorIndex + 1)
													: itemInfo);
											if (!scope.canDrop
													|| scope.canDrop({
														type : type,
